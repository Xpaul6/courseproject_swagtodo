    ...
    // Получение/создание кода
    
    app.MapGet("/familycode/{parentId}", async (int parentId, AppDbContext db) =>
        {
            var parent = await db.Users
                .Where(u => u.UserId == parentId && u.Role == "parent")
                .FirstOrDefaultAsync();

            if (parent == null)
                return Results.NotFound("Parent not found");

            var existingCode = await db.FamilyCodes
            ...
            var code = new FamilyCode
            {
                ParentId = parentId,
                Code = FamilyCode.GenerateUniqueCode()
            };

            await db.FamilyCodes.AddAsync(code);
            await db.SaveChangesAsync();
        ...
        .WithOpenApi(operation => new(operation)
        {
            Summary = "Сгенерировать код",
            Description = "Сгенерировать код для связи с ребенком",
            Tags = new List<OpenApiTag> { new() { Name = "Family" } }
        });
    
    // Ввод кода ребенком
    
    app.MapPost("/join", async (JoinRequest request, AppDbContext db) =>
        {
            var codeRecord = await db.FamilyCodes
                .FirstOrDefaultAsync(c => c.Code == request.Code);

            if (codeRecord == null)
                return Results.BadRequest("Код неверный");

            var child = await db.Users.FindAsync(request.ChildId);
        ...
        .WithOpenApi(operation => new(operation)
        {
            Summary = "Присоединитесь к родителю с помощью кода",
            Description = "Ребенок использует код для соединения с родителем",
            Tags = new List<OpenApiTag> { new() { Name = "Family" } }
        });
    
    // Получение всех детей
    app.MapGet("/children/{parentId}", async (int parentId, AppDbContext db) =>
        {
        ...
        .WithOpenApi(operation => new(operation)
        {
            Summary = "Получение всез привязанных детей",
            Tags = new List<OpenApiTag> { new() { Name = "Family" } }
        });
